...
struct InspectorView: View {
    ...
    private var lightSection: some View {
        InspectorSection("Light") {
            slider("Exposure", \.exposure, range: -5...5, step: 0.01) { String(format: "%+.2f", $0) }
            slider("Contrast", \.contrast, range: -100...100)
    ...
    private var colorSection: some View {
        InspectorSection("Color") {
            slider("Temperature", \.temperature, range: -100...100, track: [Color(red: 0.35, green: 0.55, blue: 1), Color(red: 1, green: 0.8, blue: 0.3)])
            slider("Tint", \.tint, range: -100...100, track: [Color(red: 0.3, green: 0.8, blue: 0.35), Color(red: 0.85, green: 0.35, blue: 0.85)])
            slider("Vibrance", \.vibrance, range: -100...100)
            slider("Saturation", \.saturation, range: -100...100)
        }
    }

    private func geometrySection(_ photo: Photo) -> some View {
        InspectorSection("Crop") {
            HStack {
                Text(geometrySummary(photo.settings))
    ...
    private func geometrySummary(_ settings: EditSettings) -> String {
    ...
    private func slider(
        _ title: String,
        _ keyPath: WritableKeyPath<EditSettings, Double>,
        range: ClosedRange<Double>,
        step: Double = 1,
        track: [Color]? = nil,
        format: @escaping (Double) -> String = { String(format: "%+.0f", $0).replacingOccurrences(of: "+0", with: "0") }
    ) -> some View {
        AdjustmentSlider(
            title: title,
            value: Binding(
                get: { library.activePhoto?.settings[keyPath: keyPath] ?? 0 },
                set: { newValue in
